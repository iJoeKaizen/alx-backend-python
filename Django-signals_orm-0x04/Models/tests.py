...
class MessageEditSignalTests(TestCase):
    ...
    def test_editor_tracking(self):
        """Test editor is captured when available in request"""
        # Create mock request
        request = self.factory.get('/')
        request.user = self.editor
        
        # Attach request to message class (simulate middleware)
        Message.request = request
        
        # Edit message
        self.message.content = "Edited with request"
        self.message.save()
    ...
    def test_editor_null_when_no_request(self):
        """Test editor is null when no request context"""
        self.message.content = "Edited without request"
        self.message.save()
